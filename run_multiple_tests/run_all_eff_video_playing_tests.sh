...
RES_DIR=../results/video_eff_playing_tests/$cur_date
ITER=10
schedulers=(bfq cfq)

function video_eff_playing 
{
	cd ../video_playing_vs_commands
	bash video_eff_play_vs_comms.sh $1 10 0 seq $ITER $RES_DIR

	bash video_eff_play_vs_comms.sh $1 10 0 rand $ITER  $RES_DIR

	bash video_eff_play_vs_comms.sh $1 5 5 seq $ITER $RES_DIR

	bash video_eff_play_vs_comms.sh $1 5 5 rand $ITER $RES_DIR
}
